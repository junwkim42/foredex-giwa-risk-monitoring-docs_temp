...
| Item | Demo configuration or verification location |
|---|---|
...
<!-- IMAGE_PLACEHOLDER:IMG-03 -->
> **Image placeholder IMG-03 — Demo environment overview**
>
> Identify the token, project-controlled wallet, DEX pool, and baseline values, and include a `GIWA Sepolia / demo-only` label.
...
The configuration is a demonstration DEX registered as `DEMO_V2` with `lowLiquidityDemo=true`. It must not be characterized as a production DEX or a Uniswap deployment.

When citing dashboard or on-chain values, record all of the following:

- Observation time and time zone
- Reference block
- Nature of the value: disclosed baseline, demo configuration, or on-chain observation
- Contract and addresses used

## Testnet Characteristics

GIWA Sepolia test tokens are development and validation assets with no economic value. The testnet may experience delays, errors, chain reorganizations, rollbacks, or data resets. Accordingly, the state and records of this demo do not guarantee production operating stability or economic performance.

...
> **이미지 자리 IMG-03 — 데모 환경 개요**
>
> 토큰, 프로젝트 지갑, DEX 풀과 기준값을 표시하고 `GIWA Sepolia / demo-only`를 명시하세요.
...
이 구성은 `DEMO_V2`, `lowLiquidityDemo=true`로 등록된 시연용 DEX입니다. 프로덕션 DEX 또는 Uniswap 배포로 표현하지 않습니다.

화면이나 온체인 수치를 인용할 때는 다음을 함께 기록해야 합니다.

- 관측 시각과 시간대
- 기준 블록
- 수치의 성격: 공시 기준, 데모 설정 또는 온체인 관측
- 사용한 컨트랙트와 주소

## 테스트넷 성격

GIWA Sepolia의 테스트 토큰은 경제적 가치가 없는 개발·검증용 자산입니다. 테스트넷에는 지연, 오류, 체인 재구성(chain reorganization, reorg), 롤백 또는 데이터 초기화가 발생할 수 있습니다. 따라서 이 데모의 상태와 기록은 프로덕션 운영 안정성이나 경제적 성과를 보장하지 않습니다.
